...
function ResetPassword() {
  ...
  const { reset_password, isResetPasswordLoading, hasResetPasswordError } =
    useAuth();
  ...
  useEffect(() => {
    if (
      !location.state ||
      (!location.state.fromLogin && !location.state.fromVerify)
    ) {
        ...
        <div className="bg-white w-full rounded-xl p-8 mb-8 relative flex flex-col items-center">
          ...
          <div className="flex flex-col items-center gap-1 mb-8">
            ...
            <p className="text-secondary text-sm text-center">
              Ingrese su dirección de correo electrónico y le enviaremos el
              código de verificación para restablecer su contraseña.
            </p>
          </div>
